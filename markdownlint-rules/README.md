...
**Link text** inside `[text](path)` is checked as its own phrase: it must be either valid AP title case or a file name in backticks (e.g. `` [`getting-started.md`](path) ``).
The destination `(path)` is not checked.
